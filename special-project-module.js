(function () {
    ...
    const state = { records:[], active:null, unsubscribe:null, generation:0, ready:false, page:1, rowsPerPage:10 };
    ...
    function renderList() {
        ...
        const filtered=r.filter(p=>matches(p,f));
        const totalPages=Math.max(1,Math.ceil(filtered.length/state.rowsPerPage));
        state.page=Math.min(Math.max(1,state.page),totalPages);
        const start=(state.page-1)*state.rowsPerPage;
        el('sp-rows').innerHTML=filtered.slice(start,start+state.rowsPerPage).map(p=>`<tr><td><strong>${esc(p.project_number)}</strong></td>${['created_date','project_name','area','location','pic','vendor','start_date','target_date'].map(k=>`<td>${esc(p[k]||'—')}</td>`).join('')}<td>${badge(p)}</td><td>${progress(p)}</td><td>${overdue(p)?-remaining(p)+' days':'—'}</td><td>${money(p.budget)}</td><td>${esc(date(p.updated_at))}</td><td><button type="button" data-view="${esc(p.project_number)}">View</button></td></tr>`).join('')||'<tr><td colspan="15">No projects found.</td></tr>';
        el('sp-page-info').textContent=`Showing ${filtered.length?start+1:0}–${Math.min(start+state.rowsPerPage,filtered.length)} of ${filtered.length} projects`;
        el('sp-pages').innerHTML=`<button type="button" data-page="${state.page-1}" ${state.page===1?'disabled':''}>← Previous</button>${Array.from({length:totalPages},(_,i)=>`<button type="button" data-page="${i+1}" ${state.page===i+1?'aria-current="page"':''} aria-label="Page ${i+1}">${i+1}</button>`).join('')}<button type="button" data-page="${state.page+1}" ${state.page===totalPages?'disabled':''}>Next →</button>`;
    ...
    function init() {
        if(!el('sp-app'))return;
        el('sp-app').innerHTML=`<div id="sp-message" role="status" aria-live="polite"></div><div id="sp-list"><div class="sp-header"><div><h1>Special Project</h1><p>Engineering project tracking & documentation</p></div><button id="sp-create" class="btn btn-primary" disabled>+ Create Project</button></div><div id="sp-summary"></div><div class="sp-panel"><details><summary>Search & filters</summary><form id="sp-filters"><div class="sp-grid">${filterKeys.map(k=>field(k,'',true)).join('')}</div><div class="sp-actions"><label><input type="checkbox" id="spf-overdue"> Overdue only</label><label><input type="checkbox" id="spf-completed"> Completed only</label><button type="reset">Reset filters</button></div></form></details><div class="sp-table-wrap"><table><thead><tr>${['Project Number','Created Date','Project Name','Area','Location','PIC','Vendor','Start Date','Target Date','Status','Progress','Overdue','Budget','Last Update','Action'].map(s=>`<th>${s}</th>`).join('')}</tr></thead><tbody id="sp-rows"></tbody></table></div><div class="sp-pagination"><span id="sp-page-info" role="status" aria-live="polite"></span><nav id="sp-pages" aria-label="Project pages"></nav><label for="sp-rows-per-page">Rows per page: <select id="sp-rows-per-page"><option>10</option><option>25</option><option>50</option></select></label></div></div></div><div id="sp-form" hidden></div><div id="sp-detail" hidden></div>`;
        el('sp-create').onclick=()=>showForm(); el('sp-filters').oninput=()=>{state.page=1;renderList();}; el('sp-filters').onchange=()=>{state.page=1;renderList();}; el('sp-filters').onsubmit=e=>e.preventDefault(); el('sp-filters').onreset=()=>setTimeout(()=>{state.page=1;renderList();},0);
        el('sp-rows-per-page').onchange=e=>{state.rowsPerPage=Number(e.target.value);state.page=1;renderList();};
        el('sp-app').addEventListener('click',e=>{const b=e.target.closest('button'); if(b?.hasAttribute('data-back'))view('list');if(b?.dataset.view)showDetail(b.dataset.view);if(b?.dataset.page&&!b.disabled){state.page=Number(b.dataset.page);renderList();}});
